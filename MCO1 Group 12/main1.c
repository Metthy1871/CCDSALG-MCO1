/*
    Description : serves as the main file for reading a set of points from a text file, computing
                  their convex hull using the Graham scan algorithm with bubble sort, and writing
...
Point* getPoints(strFilename filename, int *numPoints)
{
    ...
    if ((input = fopen(filename, "r")) != NULL) {
        ...
        int i;
        // read and store all points from the source file
        for(i = 0; i < *numPoints; i++) {
            fscanf(input, "%lf %lf", &pointList[i].x, &pointList[i].y); 
        }
        fclose(input); 
    } else { // source file is not found
...
int main() {
    ...
    grahamScanSlow(&stack, pointList, numPoints, &convexHullPoints);
